...
public class Spring implements Force {
    ...
    private static final float K_V = 0.35f;
    private static final float K_H = 100.4f;
    ...
    public void step(double dt, int order) {
        ...
        Vec3 dp1 = p1.norm.cross(displacement.cross(p1.norm)).normalize().times(size);
        Vec3 dp2 = p2.norm.cross(displacement.cross(p2.norm)).normalize().times(size);

        //Vec3 dn1 = displacement.cross(p1.norm.cross(displacement)).normalize();
